...
#     def add_order_data():
#         if (
#             order_idEntry.get() == '' or
#             order_dateEntry.get() == '' or
#             total_amountEntry.get() == '' or
#             payment_methodEntry.get() == '' or
#             payment_dateEntry.get() == '' or
#             customer_idEntry.get() == ''
#         ):
#             messagebox.showerror('Error', 'All fields are required', parent=add_order_window)
#         else:
#             try:
#                 # Check if the selected customer_id exists in the customers table
#                 customer_id = int(customer_idEntry.get())
#                 query = "SELECT customer_id FROM customers WHERE customer_id = %s;"
#                 mycursor.execute(query, (customer_id,))
#                 if not mycursor.fetchone():
#                     messagebox.showerror('Error', 'Invalid customer_id', parent=add_order_window)
#                     return
...
#                     payment_method, payment_date, customer_id
#                 ) VALUES (%s, %s, %s, %s, %s, %s);
#                 '''
#                 values = (
#                     order_idEntry.get(),
#                     order_dateEntry.get(),
#                     total_amountEntry.get(),
#                     payment_methodEntry.get(),
#                     payment_dateEntry.get(),
#                     customer_id
#                 )
#                 mycursor.execute(query, values)
#                 con.commit()
...
#                 result = messagebox.askyesno('Confirm', 'Do you want to clear the form?')
#                 if result:
#                     order_idEntry.delete(0, END)
#                     order_dateEntry.delete(0, END)
#                     total_amountEntry.delete(0, END)
#                     payment_methodEntry.delete(0, END)
#                     payment_dateEntry.delete(0, END)
#                     customer_idEntry.set('')  # Clear the customer_id dropdown

#             except ValueError:
#                 messagebox.showerror('Error', 'Invalid input for customer_id', parent=add_order_window)

#     add_order_window = Toplevel()
#     add_order_window.grab_set()
#     add_order_window.resizable(False, False)

#     order_idLabel = Label(add_order_window, text='Order ID', font=('Helvetica', 15, 'bold'))
#     order_idLabel.grid(row=0, column=0, padx=10, pady=10, sticky='w')
#     order_idEntry = Entry(add_order_window, font=('Helvetica', 15, 'bold'), width=24)
#     order_idEntry.grid(row=0, column=1, padx=10, pady=10)

#     order_dateLabel = Label(add_order_window, text='Order Date', font=('Helvetica', 15, 'bold'))
#     order_dateLabel.grid(row=1, column=0, padx=10, pady=10, sticky='w')
#     order_dateEntry = Entry(add_order_window, font=('Helvetica', 15, 'bold'), width=24)
#     order_dateEntry.grid(row=1, column=1, padx=10, pady=10)

#     total_amountLabel = Label(add_order_window, text='Total Amount', font=('Helvetica', 15, 'bold'))
#     total_amountLabel.grid(row=2, column=0, padx=10, pady=10, sticky='w')
#     total_amountEntry = Entry(add_order_window, font=('Helvetica', 15, 'bold'), width=24)
#     total_amountEntry.grid(row=2, column=1, padx=10, pady=10)

#     payment_methodLabel = Label(add_order_window, text='Payment Method', font=('Helvetica', 15, 'bold'))
#     payment_methodLabel.grid(row=3, column=0, padx=10, pady=10, sticky='w')
#     payment_methodEntry = Entry(add_order_window, font=('Helvetica', 15, 'bold'), width=24)
#     payment_methodEntry.grid(row=3, column=1, padx=10, pady=10)

#     payment_dateLabel = Label(add_order_window, text='Payment Date', font=('Helvetica', 15, 'bold'))
#     payment_dateLabel.grid(row=4, column=0, padx=10, pady=10, sticky='w')
#     payment_dateEntry = Entry(add_order_window, font=('Helvetica', 15, 'bold'), width=24)
#     payment_dateEntry.grid(row=4, column=1, padx=10, pady=10)

#     customer_idLabel = Label(add_order_window, text='Customer ID', font=('Helvetica', 15, 'bold'))
#     customer_idLabel.grid(row=5, column=0, padx=10, pady=10, sticky='w')
#     customer_idEntry = ttk.Combobox(add_order_window, font=('Helvetica', 15, 'bold'), state='readonly', width=22)
#     customer_idEntry.grid(row=5, column=1, padx=10, pady=10)

#     # Populate the customer_id dropdown
#     mycursor.execute("SELECT customer_id FROM customers;")
#     customers = mycursor.fetchall()
#     customer_ids = [str(c[0]) for c in customers]
#     customer_idEntry['values'] = customer_ids

#     add_order_button = Button(add_order_window, text='Add Order', font=('Helvetica', 15, 'bold'), command=add_order_data)
#     add_order_button.grid(row=6, column=0, columnspan=2, pady=10)

#     add_order_window.mainloop()
...
# addorderButton = ttk.Button(leftFrame, text='Add order', width=25, state=DISABLED, command=add_order)
